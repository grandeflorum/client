...
export class XsphbfxComponent implements OnInit {
  ...
  async setXsEchart() {
    ...
    const res = await this.statisticService.getOverallSalesTrend(option);
    if (res && res.code == 200) {
  ...
  async searchTopTen() {
    ...
    const res = await this.statisticService.getProjectSalesVolumeList(option);
    this.Loading = false;
    if (res.code == 200) {
      console.log(res);
      console.log(res.msg);
      this.rankList = res.msg;
  ...
  export() {
    window.open(AppConfig.Configuration.baseUrl + '/Statistic/excelDownload?mc=' + this.mc + '&lx=' + this.lx + '&kssj=' + this.kssj + '&jssj=' + this.jssj + '&exportType=sales');
  }
